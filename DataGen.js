 ...
 */
 ...
 async function genTalents(v, output) {
     const ClassTalents = getClassTalents((await got(util.format(LOCALE, v))).body);
     const Talents = getTalentData((await got(util.format(TALENTS, v))).body);
     const Locales = {};
     ...
     for (const [locale, id] of LOCALES) {
         Locales[locale] = getTalentLocales(
             ...
             (await got(util.format(GLOBAL, v, id))).body
         );
     }
